...
def kb_main(lang: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=t(lang, "ocr2docx"), callback_data="do:ocr2docx")],
...
def kb_translit_dir(lang: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Kiril → Lotin", callback_data="tr:cl")],
        [InlineKeyboardButton(text="Lotin → Kiril", callback_data="tr:lc")],
...
def kb_pay_kind(lang: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🧠 OCR kredit", callback_data="pay:kind:ocr")],
        [InlineKeyboardButton(text="⬅️ Menu", callback_data="menu:back")],
    ])
...
def kb_premium_plans(p7: int, p30: int, p365: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ Orqaga", callback_data="pay:back")],
    ])

def kb_ocr_plans(p1: int, p10: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"OCR 1 ta — {p1} so‘m", callback_data="pay:ocr:1")],
        [InlineKeyboardButton(text=f"OCR 10 ta — {p10} so‘m", callback_data="pay:ocr:10")],
        [InlineKeyboardButton(text="⬅️ Orqaga", callback_data="pay:back")],
    ])
